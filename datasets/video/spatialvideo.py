...
class SpatialVideoBaseDataset(BaseVideoDataset):
    """
    SpatialVideo dataset.
    ...
    def _should_download(self) -> bool:
        ...
        if self.resolution not in self._SUPPORTED_RESOLUTIONS:
            raise ValueError(
                f"Resolution {self.resolution} is not supported. Supported resolutions: {list(self._SUPPORTED_RESOLUTIONS.keys())}. Please modify `_SUPPORTED_RESOLUTIONS` in the SpatialVideoBaseDataset class to support this resolution."
            )
    ...
    def build_metadata(self, split: SPLIT) -> None:
        super().build_metadata(f"{split}_512")
        (self.metadata_dir / f"{split}_512.pt").rename(
    ...
    def load_video(
        ...
        match preprocessed_path.suffix:
            ...
            case ".mp4":
                ...
                #     end_pts=end_pts,
                # )
                video_reader = imageio.get_reader(preprocessed_path)
                num_frames = video_reader.count_frames()
                # logger.info(f"[BaseVideoDataset:load_video] Total frames to load: {num_frames}")
                frames = []
                for frame_id in range(num_frames):
                    frame = video_reader.get_data(frame_id)
                    # frame = Image.fromarray(frame)
                    # frame = self.frame_processor(frame)
                    # print(f"[LoadVideo] Processed frame {frame_id+1}/{num_frames}, size: {frame.size}")
                    frames.append(frame)
                video_reader.close()
                video = torch.from_numpy(np.stack(frames, axis=0)).float()[start_frame:end_frame]
                logger.info(f"[BaseVideoDataset:load_video] preprocessed_path: {preprocessed_path}, start_frame: {start_frame}, end_frame: {end_frame}, video shape: {video.shape}") 
                return video.permute(0, 3, 1, 2) / 255.0


class SpatialVideoSimpleDataset(
    SpatialVideoBaseDataset, BaseSimpleVideoDataset
):
    """
    SpatialVideo simple video dataset
    """
...
class SpatialVideoDataset(
    SpatialVideoBaseDataset, BaseAdvancedVideoDataset
):
    """
    SpatialVideo advanced video dataset
    """
    ...
    def _training_frame_skip(self) -> int:
        ...
        ), "Subepoch should be given to the SpatialVideoDataset, to use frame skip schedule"
    ...
    def load_cond(
        ...
        path = self.save_dir / f"{self.split}_poses" / f"{path.stem}.npz"
        data = np.load(str(path))
        w2c_poses = np.linalg.inv(data["c2w_poses"])[:, :3]  # (N, 3, 4)
        w2c_poses = w2c_poses.reshape(len(w2c_poses), -1)  # (N, 12)
        fxfycxcys = np.stack(
            [
                data["intrinsics"][:, 0, 0],  # fx
                data["intrinsics"][:, 1, 1],  # fy
                data["intrinsics"][:, 0, 2],  # cx
                data["intrinsics"][:, 1, 2],  # cy
            ], axis=-1,
        )  # (N, 4)
        # Insert 2 zero columns at positions 4-5 so that _process_external_cond
        # can uniformly apply [:4] + [6:] → (T, 16), matching the RE10K convention.
        padding = np.zeros((len(fxfycxcys), 2), dtype=np.float32)  # (N, 2)
        cond = torch.from_numpy(
            np.concatenate([fxfycxcys, padding, w2c_poses], axis=-1)  # (N, 18)
        )[start_frame:end_frame].float()
        return cond
